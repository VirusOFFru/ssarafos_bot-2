...
async def get_all_products(include_hidden: bool = False):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if include_hidden:
            ...
            query = "SELECT * FROM products WHERE in_stock = 1 ORDER BY id DESC"
            params = ()
...
async def create_order(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO orders (user_id, username, full_name, address, comment, total, delivery_cost) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (user_id, username, full_name, address, comment, total, delivery_cost),
        )
